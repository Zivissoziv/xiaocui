...
export class AiRoutingService {
  ...
  private isUsable(settings: AiSettings): boolean {
    return this.settings.isUsable(settings);
  }
  ...
  async analyze(
    ...
    if (!this.isUsable(this.loadSettings())) {
      return ruleBased.analyze(snapshot, profile, instruction, dueAt);
    }
    try {
      const result = await openAiAnalysis.analyze(this.loadSettings(), snapshot, profile, instruction, dueAt);
  ...
  async regenerateMessages(
    ...
    if (!this.isUsable(this.loadSettings())) return false;
    try {
      return await openAiAnalysis.regenerateMessages(this.loadSettings(), drafts, userInstruction, dueAt, risks);
    } catch (error) {
